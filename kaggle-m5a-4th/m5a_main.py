...
class M5aMain(object):
    ...
    def generate_grid_base(self, train_df, prices_df, calendar_df):
        ...
        # 这一步是通过apply函数对series中的每一组数据做操作，而这一操作将d_*类型的数据变为纯数字
        grid_df['d'] = grid_df['d'].apply(lambda x: x[2:]).astype(np.int16)
        # 从原表格中找出需要测试的数据部分,从训练的位置开始一直到待测试的部分
        ...
        # 释放周啥意思啊
        # 就是一些商品的一个没卖出去,实际上是没开始卖,所以要从价格表格中匹配物品和天数.只有匹配上的天数才可以保留
    ...
    # 抄的数据处理流程，用以生成三个版本的后处理数据，分别是完整的销售数据、加上价格的数据、加上日期信息的数据
    ...
    def main(self):
        ...
        # 给1941这个数单独一个文件夹，应该代表从1941天开始进行预测吧。
